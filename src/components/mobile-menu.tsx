...
export default function MobileMenu({ isOpen, onToggle }: MobileMenuProps) {
  ...
  const handleWhatsApp = () => {
    const message = "Hello Ralhum Sports! I'm interested in your sports equipment. Please contact me."
  ...
  return (
    <>
      {/* Overlay */}
      {isOpen && <div className="fixed inset-0 bg-black/50 z-40 md:hidden" onClick={onToggle} aria-hidden="true" />}
      ...
      >
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-200">
          ...
          <Button variant="ghost" size="sm" onClick={onToggle} aria-label="Close menu" className="p-2">
            <X className="w-6 h-6" />
          </Button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto py-4">
          <nav className="space-y-2 px-4">
            {navItems.map((item) => (
              ...
              >
                <div className="font-bold text-gray-900 group-hover:text-[#003DA5] transition-colors">{item.name}</div>
          ...
          <div className="mt-8 px-4 py-4 border-t border-gray-200">
            <Badge className="bg-[#FFD700] text-[#1A1A1A] font-bold">Sri Lanka's #1 Sports Distributor</Badge>
            <p className="text-xs text-gray-500 mt-2">25+ Years of Excellence • Exclusive Brand Partnerships</p>
          </div>
        </div>
      </div>
    </>
