    ...
    beforeChange: [
      async ({ req, operation, data }) => {
        ...
        if (data.orderItems && Array.isArray(data.orderItems)) {
          ...
          data.orderSubtotal = subtotal

          // Calculate final total
          const shipping = data.shippingCost || 0
          const discount = data.discount || 0
          data.orderTotal = subtotal + shipping - discount
        }

        // Set WhatsApp message timestamp when message is marked as sent
        if (data.whatsapp?.messageSent && !data.whatsapp?.messageTimestamp) {
          data.whatsapp.messageTimestamp = new Date()
        }
