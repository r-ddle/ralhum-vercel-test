...
import { getAuthContext, requireAdminOrManager } from '@/lib/auth'
import { withRateLimit, rateLimitConfigs } from '@/lib/rate-limit'
import { filterOrderData, createFilteredResponse, getSecurityHeaders } from '@/lib/response-filter'
  ...
  async (request: NextRequest) => {
    ...
    try {
      ...
      try {
        ...
        console.error('Error creating order:', orderError, JSON.stringify(orderError, null, 2))
        return NextResponse.json(
          {
            success: false,
            error:
              orderError && typeof orderError === 'object' && 'message' in orderError
  ...
  requireAdminOrManager(async (request: NextRequest, auth) => {
    try {
      ...
      const filteredOrders = result.docs
        .map((order) => filterOrderData(order, auth))
