...
export function formatWhatsAppMessage(order: OrderSummary): string {
  ...
  const productList = items
    .map((item) => {
      ...
      return `• ${item.product.title} (${item.variant.name}) x${item.quantity} - ${formatLKR(itemTotalLKR)}`;
    })
    .join("\n");
...
*Pricing:*
Subtotal: ${formatLKR(pricing.subtotal)}
Shipping: ${formatLKR(pricing.shipping)}
Tax (15%): ${formatLKR(pricing.tax)}
*Total: ${formatLKR(pricing.total)}*
